...
define('entity/thng',[
  ...
  // Custom nested resource constructor for Thngs of a Thngs.
  function thng(id) {
    // To create nested Resources, the thng itself needs
    // a resource.
    if(!this.resource) {
      throw new Error('This Entity does not have a Resource.');
    }
  ...
  return {
    ...
    resourceConstructor: Resource.constructorFactory('/thngs', EVT.Entity.Thng, ['property', 'action', 'location', 'share', 'thng'])
...
define('entity/reactorLog',[
  ...
  return {
    ...
    resourceConstructor: function (id) {
      ...
      if (hasResource) {
        path = this.resource.path + '/reactor/logs';
      } else {
        path = '/projects/' + this.project + '/applications/' + this.id + '/reactor/logs';
      }
...
define('entity/reactorScript',[
  ...
  var ReactorScript = function () {
    Entity.apply(this, arguments);
  };

  ReactorScript.prototype = Object.create(Entity.prototype);
  ReactorScript.prototype.constructor = ReactorScript;
  ...
  return {
    ...
    resourceConstructor: function (id) {
      if (id) {
        // Reactor scripts cannot be retrieved/updated individually
        throw new TypeError('IDs not allowed here');
      }
      ...
      if (hasResource) {
        path = this.resource.path + '/reactor/script';
      } else {
        path = '/projects/' + this.project + '/applications/' + this.id + '/reactor/script';
      }
...
// **The ReactorSchedule Entity maps to the reactor schedules API,
// allowing the same set of operations.**

define('entity/reactorSchedule',[
  'core',
  './entity',
  'resource',
  'scope/scope',
  ...
  // Setup ReactorSchedule inheritance from Entity.
  var ReactorSchedule = function () {
    Entity.apply(this, arguments);
  };

  ReactorSchedule.prototype = Object.create(Entity.prototype);
  ReactorSchedule.prototype.constructor = ReactorSchedule;


  // Attach class to EVT module.
  EVT.Entity.ReactorSchedule = ReactorSchedule;


  return {

    'class': ReactorSchedule,
    ...
    resourceConstructor: function (id) {
      ...
      if (hasResource) {
        path = this.resource.path + '/reactor/schedules';
      } else {
        path = '/projects/' + this.project + '/applications/' + this.id + '/reactor/schedules';
      }

      // Create a resource constructor dynamically and call it.
      resource = Resource.constructorFactory(path, ReactorSchedule).call(scope, id);
...
// **The Reactor is a namespace (i.e. an endpoint without methods, used just
// to group other related resources, in this case Reactor Logs, Reactor Scripts
// and Reactor Schedule. Namespaces should pass the same parent resource
// context down to the endpoints.**

define('namespaces/reactor',[
  '../entity/reactorLog',
  '../entity/reactorScript',
  '../entity/reactorSchedule'
], function (ReactorLog, ReactorScript, ReactorSchedule) {
  'use strict';

  var Reactor = function (context) {

    this.log = ReactorLog.resourceConstructor.bind(context);

    this.script = ReactorScript.resourceConstructor.bind(context);

    this.schedule = ReactorSchedule.resourceConstructor.bind(context);
...
define('entity/application',[
  ...
  '../namespaces/reactor',
  'resource'
], function (EVT, Entity, Reactor, Resource) {
  'use strict';
  ...
  var Application = function () {
    ...
    // Reactor is a namespace that is bound to this instance.
  ...
  return {
    ...
    resourceConstructor: function (id) {
      ...
      resource = Resource.constructorFactory(path, EVT.Entity.Application).call(scope, id);

      // Similar to nested resources, but namespaces are not functions.
      var rawEntity = new Entity({id: id}, resource);
      resource.reactor = new Reactor(rawEntity);
...
define('entity/task',[
  ...
  return {
    ...
    resourceConstructor: function (id) {
      ...
      return Resource.constructorFactory(path, EVT.Entity.Task).call(scope, id);
    }
  };
});
...
// **Batch is a simple Entity subclass that provides a nested
// Task Resource.**
...
define('entity/batch',[
  ...
  'resource',
  'utils'
], function (EVT, Entity, Task, Resource, Utils) {
  'use strict';

  // Setup Batch inheritance from Entity.
  var Batch = function () {
    Entity.apply(this, arguments);
  };

  Batch.prototype = Object.create(Entity.prototype);
  Batch.prototype.constructor = Batch;

  // Attach class to EVT module.
  EVT.Entity.Batch = Batch;

  // Extend Batch API by exposing a Task Resource.
  Utils.extend(Batch.prototype, {

    task: Task.resourceConstructor
  ...
  return {
    ...
    resourceConstructor: Resource.constructorFactory('/batches', EVT.Entity.Batch, ['task'])
...
// **The File is a simple Entity subclass representing the REST API
// File object.**
define('entity/file',[
  'core',
  './entity',
  ...
  // Setup File inheritance from Entity.
  var File = function () {
    Entity.apply(this, arguments);
  };

  File.prototype = Object.create(Entity.prototype);
  File.prototype.constructor = File;

  // Attach class to EVT module.
  EVT.Entity.File = File;
  ...
  return {
    ...
    resourceConstructor: function (id) {
      var resource = Resource.constructorFactory('/files', EVT.Entity.File).call(this, id);
...
// - Action resource (`C`, `R`, `U`, `D`)
// - Collection resource (`C`, `R`, `U`, `D`)
...
define('scope/operator',[
  ...
  // - _**new EVT.Operator(apiKey)** - Operator API Key string._
  // - _**new EVT.Operator(options)** - Options object should
  // contain `apiKey` and optionally operator information._
  var OperatorScope = function (obj) {
    ...
    if (Utils.isObject(obj)) {
      ...
      // Merge operator information into the scope, as we do with the ApplicationScope.
      Utils.extend(this, obj, true);
    ...
    // Get operator information asynchronously from the Engine using already
    // defined scope. Use **new EVT.Operator('apiKey').$init.then(success)** if need
    // to wait for operator information.
    this.$init = this.$init.then(function(access) {
      // TODO operators should be able to read themselves - BE not implemented.
      ...
      return Utils.extend($this, {account: accounts[0]}, true);
    }).catch(function () {
      Logger.error('There is no operator with this API Key.');
    });
  };


  // Setup Scope inheritance.
  OperatorScope.prototype = Object.create(Scope.prototype);
  OperatorScope.prototype.constructor = OperatorScope;
  ...
  Utils.extend(OperatorScope.prototype, {
    ...
    batch: Batch.resourceConstructor,
...
define('scope/trustedApplication',[
  ...
  'namespaces/reactor',
  'utils'
], function (EVT, ApplicationScope, User, Thng, ActionType, Collection, Reactor, Utils) {
  'use strict';
...
// # **[EVRYTHNG](https://www.evrythng.com)'s JavaScript SDK (Extended)**
...
define('evrythng-extended',[
  ...
  return EVT;
    ...
    //value.
    return require('evrythng-extended');
}));
