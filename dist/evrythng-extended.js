...
define('entity/property',[
  ...
  function _normalizeArguments(args) {
    ...
    if(Utils.isString(data) || typeof data === 'number'){
...
// **Here it is defined the AccountScope or `EVT.Account`. EVT.Account
// is a sub-class of scope and it defines the public API that the
// operator can access.**

// An Account scope currently has access to:
...
define('scope/account',[
  ...
  'logger'
             ...
             Multimedia, Utils, Logger) {
  

  // Account Scope constructor. It can be called with the parameters:

  // - ***new EVT.Account(apiKey)** - Operator API Key string.
  // - ***new EVT.Account(options)** - Options object should
  // contain `apiKey` and optionally account information.*
  var AccountScope = function(obj){
    ...
    if(Utils.isObject(obj)){
      ...
      // Merge account information into the scope, as we do with the ApplicationScope.
    ...
    // Get account information asynchronously from the Engine using already
    // defined scope. Use **new EVT.Account('apiKey').$init.then(success)** if need
    // to wait for account information.
    this.$init = EVT.api({
      url: '/accounts',
      authorization: this.apiKey
    }).then(
      function (accounts) {
        return Utils.extend($this, accounts[0], true);
      }, function () {
        Logger.error('There is no account with this API Key.');
      }
  ...
  AccountScope.prototype = Object.create(Scope.prototype);
  ...
  Utils.extend(AccountScope.prototype, {
  ...
  // Attach AccountScope class to the EVT module.
  EVT.Account = AccountScope;
